...
async function sendEmail(env: Env, payload: ContactPayload): Promise<void> {
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (context.env.TURNSTILE_SECRET_KEY && payload.turnstileToken) {
      const ip = context.request.headers.get('CF-Connecting-IP') || '';
      const valid = await verifyTurnstile(payload.turnstileToken, context.env.TURNSTILE_SECRET_KEY, ip);
      if (!valid) {
        return new Response(
    ...
    await sendEmail(context.env, payload);
